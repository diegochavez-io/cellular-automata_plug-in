...
must_haves:
  truths:
    ...
    - "pipeline.py uses BasePipelineConfig with Pydantic fields + ui_field_config when Scope API is available (PLUG-02); CAPipeline inherits Pipeline ABC with get_config_class() when available (PLUG-03); falls back to plain class with static ui_field_config() dict when Scope is not installed"
    ...
    - "Shared _ca_call() helper avoids code duplication between formal and fallback CAPipeline branches"
  ...
  artifacts:
    ...
    - path: "plugins/cellular_automata/pipeline.py"
      provides: "Dual-pattern CAPipeline: BasePipelineConfig + Pipeline ABC when Scope available, plain class fallback otherwise; shared _ca_call() helper"
...
<task type="auto">
  ...
  <action>
Create `plugins/cellular_automata/pipeline.py` with a **dual-pattern** CAPipeline class that uses `BasePipelineConfig` + `Pipeline` ABC when the formal Scope API is available, falling back to a plain class for local dev without Scope installed. Per STATE.md locked decision: "Scope plugin uses BasePipelineConfig with Pydantic fields + ui_field_config".
...
- BasePipelineConfig with Pydantic fields + json_schema_extra=ui_field_config() (PLUG-02)
- CAPipeline inherits Pipeline ABC with get_config_class() (PLUG-03)
- Shared `_ca_call()` helper eliminates code duplication between formal and fallback branches
...
- **Dual-pattern (PLUG-02/PLUG-03):** When `scope.core.pipelines.*` is importable, uses `CAPipelineConfig(BasePipelineConfig)` with Pydantic `Field` + `json_schema_extra=ui_field_config(...)`, and `CAPipeline(Pipeline)` with `get_config_class()`. When Scope is not installed (local dev), uses a plain `CAPipeline` class with `@staticmethod ui_field_config()` returning a dict.
- **No code duplication:** `_ca_init()` and `_ca_call()` are module-level helper functions used as method bodies by both branches via `__init__ = _ca_init` / `__call__ = _ca_call`.
- **Preset list:** `_PRESET_CHOICES` is a module-level constant shared by both branches. Contains ALL headless-safe engine presets as internal keys that `CASimulator.apply_preset()` expects.
- **Warmup on preset switch:** `apply_preset()` skips warmup (due to `self._warmup=False` in simulator), so `run_warmup()` is called explicitly after preset switch.
  </action>
  <verify>
1. Run the headless verification script (works in both Scope-installed and fallback modes):
```bash
...
"
```
2. Verify dual-pattern structure and shared helpers exist:
```bash
cd /Users/agi/Code/daydream_scope/plugins && python3 -c "
from cellular_automata import pipeline as mod

# Shared helpers must exist at module level
assert hasattr(mod, '_ca_init'), 'Missing _ca_init helper'
assert hasattr(mod, '_ca_call'), 'Missing _ca_call helper'
assert hasattr(mod, '_HAS_SCOPE_API'), 'Missing _HAS_SCOPE_API flag'
assert hasattr(mod, '_PRESET_CHOICES'), 'Missing _PRESET_CHOICES list'

# CAPipeline must exist regardless of Scope availability
assert hasattr(mod, 'CAPipeline'), 'Missing CAPipeline class'

# If Scope API is available, CAPipelineConfig must also exist
if mod._HAS_SCOPE_API:
    assert hasattr(mod, 'CAPipelineConfig'), 'Missing CAPipelineConfig'
    cfg = mod.CAPipelineConfig
    assert cfg.pipeline_id == 'cellular-automata'
    assert cfg.pipeline_name == 'Cellular Automata'
    assert cfg.supports_prompts is False
    assert hasattr(mod.CAPipeline, 'get_config_class')
    print('Formal Scope API branch: CAPipelineConfig + Pipeline ABC OK')
else:
    # Fallback must have ui_field_config()
    config = mod.CAPipeline.ui_field_config()
    assert 'preset' in config and 'speed' in config and 'sim_size' in config
    assert config['sim_size'].get('is_load_param') is True
    print('Fallback branch: plain CAPipeline with ui_field_config() OK')

print('Dual-pattern structure OK')
"
```
...
```
  ...
  <done>
- pipeline.py exists with dual-pattern CAPipeline: BasePipelineConfig + Pipeline ABC when Scope API available (PLUG-02, PLUG-03), plain class fallback otherwise
- CAPipelineConfig has pipeline_id, pipeline_name, supports_prompts, modes, and all 7 Pydantic fields with json_schema_extra=ui_field_config(...)
- CAPipeline(Pipeline) implements get_config_class() returning CAPipelineConfig
- Shared _ca_init() and _ca_call() module-level helpers eliminate code duplication between branches
...
- Fallback ui_field_config() returns complete UI layout with preset choices, sliders, and toggle
- _PRESET_CHOICES module constant contains all headless-safe engine presets (internal keys)
...
# Verify pipeline.py loads and dual-pattern structure
from cellular_automata import pipeline as mod
from cellular_automata.pipeline import CAPipeline
print('PLUG-03: CAPipeline imported OK')

# Verify dual-pattern: check which branch is active
assert hasattr(mod, '_HAS_SCOPE_API'), 'Missing _HAS_SCOPE_API flag'
assert hasattr(mod, '_ca_init') and hasattr(mod, '_ca_call'), 'Missing shared helpers'

if mod._HAS_SCOPE_API:
    # Formal Scope API available (PLUG-02 + PLUG-03)
    assert hasattr(mod, 'CAPipelineConfig'), 'Missing CAPipelineConfig'
    cfg = mod.CAPipelineConfig
    assert cfg.pipeline_id == 'cellular-automata'
    assert cfg.pipeline_name == 'Cellular Automata'
    assert cfg.supports_prompts is False
    assert hasattr(CAPipeline, 'get_config_class')
    assert CAPipeline.get_config_class() is mod.CAPipelineConfig
    print('PLUG-02: CAPipelineConfig (BasePipelineConfig) OK')
    print('PLUG-03: CAPipeline(Pipeline) with get_config_class() OK')
else:
    # Fallback: plain class with ui_field_config()
    ...
    print('PLUG-02: ui_field_config (fallback) OK')
    print('PLUG-03: CAPipeline (plain class fallback) OK')
...
7. Dual-pattern: CAPipelineConfig(BasePipelineConfig) + CAPipeline(Pipeline) with get_config_class() when Scope API available (PLUG-02, PLUG-03); plain CAPipeline with ui_field_config() dict when not
8. Shared _ca_init() and _ca_call() helpers — no duplicated __init__/__call__ logic between branches
</success_criteria>
