...
must_haves:
  truths:
    - "plugin.py imports CAPipeline and registers it via register_pipelines(registry)"
...
Create `plugins/cellular_automata/plugin.py` following the **local example_plugin pattern** (confirmed working):
...
Use the simpler local pattern (no @hookimpl) — this is the confirmed working pattern from the example_plugin in the repo. The requirements specify @hookimpl but research recommends starting with the simpler pattern per user decision to follow the working local example. If Scope rejects this pattern, upgrade to @hookimpl in a gap closure plan.
  </action>
  <verify>
...
- plugin.py registers CAPipeline via registry.register()
